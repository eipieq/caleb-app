...
| `/strategy` | strategy config form. wallet-gated. toggle agent on/off, pick strategy (momentum / mean-revert), set max spend, confidence threshold, allowed tokens, max position, max drawdown. saves per-address. | `GET /api/policy`, `POST /api/policy` |
...
| route | purpose |
|---|---|
| `/api/verify/[id]` | proxies `AGENT_API/api/verify/:id` to avoid mixed-content (HTTP VPS from HTTPS page) |
| `/api/attestations/[id]` | proxies `AGENT_API/api/attestations/:id`, same reason |
...
- `strategy` — which algorithm was used (momentum, mean-revert)
...
the next.js API routes exist solely to proxy HTTP VPS endpoints so the HTTPS-served frontend avoids mixed-content errors.
...
the VPS serves over HTTP. the app is deployed on Vercel over HTTPS. browsers block mixed-content fetches. the two Next.js API routes (`/api/verify/[id]`, `/api/attestations/[id]`) are server-side proxies that sidestep this.
...
- **no websocket / SSE.** polling every 10s. fine for hourly cron frequency, would need real-time transport for actual HFT.
- **legacy sessions can't be verified.** hash format mismatch. they're displayable but verification returns false positives.
- **chain commit failures.** nonce race conditions between overlapping sessions caused some trades to record in the portfolio but not commit on-chain. fixed in the runner, but affected sessions show a yellow warning banner.
